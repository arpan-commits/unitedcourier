        ...
        <div class="page-wrapper">
            <div class="content pb-0">
                ...
                <div class="d-flex align-items-center justify-content-between gap-2 mb-4 flex-wrap">
                    <div>
                        <h4 class="mb-1">Partnership Forms</h4>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title">Partnership Form Requests</h5>
                                <p class="card-text">View all partnership requests submitted through the website</p>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table class="table table-hover" id="formsTable">
                                        ...
                                        <tbody>
                                            @forelse($forms as $form)
                                                <tr>
                                                    <td>{{ $form->id }}</td>
                                                    <td><strong>{{ $form->first_name }}
                                                            {{ $form->last_name }}</strong></td>
                                                    <td>{{ $form->email }}</td>
                                                    <td>{{ $form->phone }}</td>
                                                    <td>{{ $form->company_name }}</td>
                                                    <td>{{ $form->message ?? '—' }}</td>
                                                    <td>{{ $form->created_at ? $form->created_at->format('d M Y, h:i A') : 'N/A' }}
                                                    </td>
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="8" class="text-center py-4">
                                                        <p class="text-muted">No partnership requests yet.</p>
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
